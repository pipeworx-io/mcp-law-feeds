 ...
 * Law Feeds MCP.
 *
 * Law & courts — legal news, national-security law and the US Supreme Court.
...
const FEEDS: Record<string, FeedDef> = {
  ...
  'above-the-law': { title: "Above the Law", url: "https://abovethelaw.com/feed/", category: "law", source: "Above the Law", description: "Legal industry news." },
  volokh: { title: "The Volokh Conspiracy", url: "https://reason.com/volokh/feed/", category: "law", source: "The Volokh Conspiracy", description: "Law & legal commentary." },
};

const tools: McpToolExport['tools'] = [
  {
    name: 'list_feeds',
    description: 'List the curated law & courts feeds (id, title, category, source). Optionally filter by category (government, law) or keyword. Pass an id to read_feed.',
    inputSchema: {
      type: 'object',
      properties: {
        category: { type: 'string', description: 'Filter by category: government, law.' },
  ...
  {
    name: 'read_feed',
    description: 'Read a curated law & courts feed by its id (from list_feeds). Returns normalized items (title, link, published, summary). Optionally filter items by keyword.',
    inputSchema: {
      type: 'object',
      properties: {
